...
class Logger:
    ...
    def dump(self):
        ...
        directory = '_'.join(['LAC', 'results'])
        filename1 = '_'.join(['LAC', constraint, envname, 'log_data_seed', str(seed)]) + '.pkl'
        filename2 = '_'.join(['LAC', constraint, envname, 'hyperparams_seed', str(seed)]) + '.pkl'
        filename3 = '_'.join(['LAC', constraint, envname, 'models_seed', str(seed)]) + '.pth'
